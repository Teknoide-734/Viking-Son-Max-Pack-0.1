...
for file in "$INPUT_DIR"/*.wav; do
    ...
    # ➤ Lancer Demucs (modèle 2 stems : vocals + no_vocals)
    demucs --two-stems=vocals "$file"

    # ➤ Chemin généré automatiquement par Demucs
    DEMUCS_OUT_DIR="$HOME/separated/htdemucs/$base_name"

    # ➤ Copie et renommage des pistes dans le dossier de sortie
    cp "$DEMUCS_OUT_DIR/vocals.wav"     "$OUTPUT_DIR/${base_name}-vocals.wav"
    cp "$DEMUCS_OUT_DIR/no_vocals.wav" "$OUTPUT_DIR/${base_name}-no-vocals.wav"
